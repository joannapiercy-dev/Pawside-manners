  ...
  try {
    ...
    if (profiles && events) {
      ...
      const rows = sorted.map((p, i) => {
        ...
        return \`<tr style="border-bottom:1px solid var(--warm-mid);\${isMe ? 'background:#fefce8;font-weight:600;' : i % 2 === 0 ? '' : 'background:var(--warm);'}">
          <td style="padding:10px 14px;font-size:13px;color:var(--ink-light);width:36px;">\${medal}</td>
          <td style="padding:10px 14px;">
            <div style="font-size:13.5px;color:var(--ink);">\${p.full_name}\${isMe ? ' <span style=\"font-size:11px;background:#fde047;border-radius:10px;padding:1px 7px;color:#a16207;\">you</span>' : ''}</div>
      ...
      leaderboardHtml = \`
        <div style="background:white;border:1px solid var(--warm-mid);border-radius:var(--radius-lg);overflow:hidden;margin-top:1.5rem;">
