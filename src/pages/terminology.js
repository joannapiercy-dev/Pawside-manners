...
function setupFlashcards(deck, idx, render) {
  ...
  document.getElementById('prev-btn')?.addEventListener('click', () => {
    if (currentIdx > 0) render('flashcards', currentIdx - 1);
  });

  document.getElementById('next-btn')?.addEventListener('click', () => {
    if (currentIdx < deck.terms.length - 1) render('flashcards', currentIdx + 1);
  });

  document.getElementById('shuffle-btn')?.addEventListener('click', () => {
    // Shuffle the deck order and re-render from 0
  ...
  document.onkeydown = (e) => {
    if (e.key === 'ArrowRight' && currentIdx < deck.terms.length - 1) render('flashcards', currentIdx + 1);
    if (e.key === 'ArrowLeft' && currentIdx > 0) render('flashcards', currentIdx - 1);
    if (e.key === ' ') { e.preventDefault(); currentFlipped = !currentFlipped; render('flashcards', currentIdx); }
  };
}
